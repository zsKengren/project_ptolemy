    ...
    <div ref="container" id="picture-input" class="picture-input">
      ...
      <div v-else-if="supportsPreview">
        <UContainer class="preview-container"
          ...
          <UContainer v-if="!imageSelected && !plain"
            class="picture-inner border border-dashed border-gray-600"
        ...
        <UButton v-if="imageSelected && !hideChangeButton" @click.prevent="selectImage" >{{ strings.change }}</UButton>
        <UButton v-if="imageSelected && removable" @click.prevent="removeImage" color="gray" variant="ghost">{{ strings.remove }}</UButton>
        <UButton v-if="imageSelected && toggleAspectRatio && width !== height" @click.prevent="rotateImage" color="gray" variant="ghost">{{ strings.aspect }}</UButton>
      </div>
      <div v-else>
        <UButton v-if="!imageSelected" @click.prevent="selectImage">{{ strings.select }}</UButton>
        <div v-else>
          <div v-html="strings.selected"></div>
          <UButton v-if="!hideChangeButton" @click.prevent="selectImage">{{ strings.change }}</UButton>
          <UButton v-if="removable" @click.prevent="removeImage" color="gray" variant="ghost" >{{ strings.remove }}</UButton>
        </div>
      </div>
